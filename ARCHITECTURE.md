...
| `run_workflow.py` | **Orchestrator — re-runnable workflow entry point.** Accepts `--source playstore` (with `--app-id`, `--count`) or `--source csv` (with `--path`, `--max-rows`). Clears all pipeline artifacts, scrapes/imports, then calls clean → tag → embed → aggregate → rag in sequence. Exposes `run_pipeline()` for the Streamlit Tab 3 UI. Sets `TAG_ALL_SOURCES=1` for tag.py and `PLAYSTORE_APP_ID` for scrape_playstore.py. | CLI args or function call | fresh `data/insights/*.json` + `chroma_db/` |
...
| `embed.py` | Filters `tag_error=True` rows (~100), embeds remaining ~5,708 rows using `sentence-transformers all-MiniLM-L6-v2` (local CPU, no API). Batches of 64. Upserts into ChromaDB persistent collection `spotify_reviews` (cosine space) with metadata: source, rating, score, sentiment, segment, themes (comma-joined str), discovery_related (int), tagged_by, language. Runs 3 test similarity queries on completion. | `data/tagged/reviews_tagged.parquet` | ChromaDB at `./chroma_db/` |
...
| 4 | Embed (sentence-transformers → Chroma) | **Done** | ~5,708 vectors (tag_error rows dropped). Model: `all-MiniLM-L6-v2` (local). Collection: `spotify_reviews`, cosine space. Persist path: `./chroma_db/`. Metadata includes score for Q6 weighting. |
| 5 | Aggregate + six-question answers | **Done** | aggregate.py → summary.json (36 KB): theme counts, sentiment, segment × theme cross-tab, score-weighted unmet needs. rag.py → answers.json (20 KB): all 6 questions answered, 0 GENERATION_FAILED. Top theme: discovery_friction (1,104 rows). Highest discovery_friction segment: power_user (28.2%). Top unmet need by weighted score: discovery_friction (2,185). |
| 6 | Streamlit app + Re-runnable Workflow | **Done** | `app.py` three tabs: Tab 1 (Insights Dashboard), Tab 2 (Ask the Reviews RAG), Tab 3 (Run Workflow). Tab 3 accepts Play Store app ID or CSV upload, runs the full pipeline via `run_workflow.run_pipeline()` with per-stage progress bar, then renders fresh results inline. `run_workflow.py`: CLI + callable orchestrator; supports `--source playstore` and `--source csv`; sets `TAG_ALL_SOURCES=1` for tag.py and `PLAYSTORE_APP_ID` for scraper. `clean.py` updated with file-existence guards (missing sources return empty list) and CSV loader. `tag.py` respects `TAG_ALL_SOURCES=1` env var to tag all sources in workflow mode. |
| 7 | Deploy (public URL) | **Done** | Streamlit Community Cloud. chroma_db/ committed to repo (29.3 MB, pre-built index). requirements.txt pins chromadb==1.0.0, sentence-transformers==3.4.1, torch==2.3.1+cpu (CPU-only wheel via --extra-index-url). .streamlit/config.toml sets headless=true + light theme. GROQ_API_KEY set under app Secrets. Public URL: TBD |
