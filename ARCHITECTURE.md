...
| `retag_errors.py` | Re-tags all `tag_error=True` rows using Gemini 2.5 (google-genai SDK). Primary: `gemini-2.5-flash-lite`; fallback: `gemini-2.5-flash`. Honors API-supplied `retryDelay` on 429s. 4 s inter-call sleep (~15 RPM). Overwrites only error rows; good rows untouched. Checkpoints every 50 rows. | `reviews_tagged.parquet` | `reviews_tagged.parquet` (updated in-place) |
