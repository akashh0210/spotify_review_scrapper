...
| 6 | Streamlit app | **Done** | `app.py`: Tab 1 (Insights Dashboard) loads from summary.json + answers.json only — no API calls on load. Tab 2 (Ask the Reviews) does live ChromaDB retrieval + Groq 70B streaming on submit. Groq key via `st.secrets["GROQ_API_KEY"]` with `.env.local` fallback. Spotify dark theme: `#121212` bg, `#1DB954` accent, Spotify SVG logo in header, Title-cased all display labels (segments, themes, sentiments). `.streamlit/config.toml` set to `base = "dark"` with Spotify brand colours. |
